...
export default function MonthlyAttendanceView({ 
  ...
  const handleSetStatus = (newStatus) => {
    ...
    const { empId, dateStr } = editingCell;
          ...
          <table className="w-full border-collapse text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {filteredEmployees.length === 0 ? (
                <tr>
                  <td colSpan={monthDays.length + 6} className="p-12 text-center text-slate-400">
                      ...
                      {monthDays.map((d) => {
                        ...
                        return (
                          <td 
                            ...
                            title={`${emp.name} on ${d.dateStr}: ${status.toUpperCase()} (Click to change)`}
                          >
                            {status === 'present' && (
